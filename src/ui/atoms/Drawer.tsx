...
export function Drawer({ children, header, onClose, open }: Props) {
	return (
		...
		>
			<div
				id="drawer-bottom"
				className={`${open ? "translate-y-0" : "translate-y-full"} duration-250 bg-secon fixed bottom-0 left-0 z-40 min-h-[600px] w-full transform overflow-y-hidden rounded-t-2xl bg-primary-background p-4 text-secondary transition-all ease-linear`}
				aria-labelledby="bottom-drawer"
			>
				<ButtonIcon variant="text" className="absolute right-2 top-2 ">
					<Icons.close className="text-secondary" />
				</ButtonIcon>
